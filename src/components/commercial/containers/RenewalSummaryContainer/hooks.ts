...
const MANIFEST_QUERY_KEY = ["renewal-summary-manifest"]
const CHAT_SESSION_KEY = "boxwood_renewal_summary_chat_session"

export const useRenewalSummaryManifest = () => useQuery({
  queryKey: MANIFEST_QUERY_KEY,
  queryFn: AppActions.getRenewalSummaryManifest,
})

export const useDownloadRenewalSummaryFile = () => useMutation({
...
export const useDeleteRenewalSummaryFile = () => {
  ...
  return useMutation({
    mutationFn: AppActions.deleteRenewalSummaryFile,
...
export const useRenewalSummaryChat = () => {
  const queryClient = useQueryClient()
  const [messages, setMessages] = useState<AppTypes.RenewalSummaryChatMessage[]>([])
  const [sessionId, setSessionId] = useState<string | undefined>(() => sessionStorage.getItem(CHAT_SESSION_KEY) ?? undefined)
  const [scrollSignal, setScrollSignal] = useState(0)

  const { mutate, isPending } = useMutation({
    mutationFn: (message: string) => AppActions.postRenewalSummaryChat(message, sessionId),
    onSuccess: async (turn) => {
      ...
      const builtSummary = turn.tool_calls.some((call) => call.name.endsWith("renewal_summary"))
      if(builtSummary) {
...
export const useHandleCsrGroupList = (groups: AppTypes.RenewalSummaryCsrGroup[], scrollSignal: number) => {
...
export const useHandleCsrSection = (summaries: AppTypes.RenewalSummaryManifestEntry[], order: string) => {
